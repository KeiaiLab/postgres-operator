...
| F02   | instance manager P2-T3+ — postgres 프로세스 supervise + promote/demote 실장 | 구현 | 60% | F01b,T15 | F03 | 2026-05-03. supervise/ 패키지 (Real+Mock+SQL+pdeathsig+fake-postgres testdata, ~1170 LoC) + cmd/instance/main.go wiring (buildSupervisor/startSupervisor/gracefulStopSupervisor + buildFencer 추출, OnStartedLeading 안 sup.Promote, OnStoppedLeading 안 sup.Stop fast). lint 0 / 모든 패키지 test PASS. 후속 (F02 100%): controller Pod env 주입 (POSTGRES_BIN_DIR/DATA_DIR/CONFIG_FILE/HBA_FILE/LOCAL_DSN), readiness HTTP endpoint, WAL lag 측정. |
