...
3. **phase 별 mergeable 증분** (#216/#217 phase-merge 패턴 정합 — 각 phase 가 독립 PR):
   ...
   - **P2**: `Bootstrap` phase 가 target shard 의 StatefulSet + headless Service + ConfigMap 을 격리 식별로 생성. fake-client unit test (Bootstrap 후 N target STS 존재 단언). 가역(rollback=target STS delete).
