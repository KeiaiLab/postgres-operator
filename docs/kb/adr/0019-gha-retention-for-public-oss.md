# ADR-0019: GitHub Actions 유지 — operator family v2.0 통합 정합 (postgres-operator)
...
| Related | ADR-0017 (이전 retention rationale — Superseded by ADR-0018), valkey ADR-0048 (sister), mongodb-operator ADR-0033 (sister, S7 cycle 동시 작성) |
...
- ADR-0018 의 *전면 제거* 결정을 1일 (2026-05-21) 만에 다시 뒤집음 — governance 안정성 측면에서 흔들림 신호. mitigation: operator family v2.0 통일 결정으로 향후 안정성 ↑.
...
- mongodb-operator ADR-0033 (S7 cycle 동시 작성) 와 sister 관계 — 두 ADR 이 cross-reference.
...
2. **ADR-0018 유지 (strict 노선 견지)** — Rejected. operator family v2.0 통일 결정에 위반. valkey ADR-0048 와의 정책 분기 유지 시 maintainer 인지 비용 ↑.
...
- **Sister ADRs (cross-operator)**:
  - `valkey-operator/docs/kb/adr/0048-gha-retention-for-public-oss.md` (Accepted, 2026-05-21) — 본 ADR 의 sister/원형.
  - `mongodb-operator/docs/kb/adr/0033-gha-retention-for-public-oss.md` (Accepted, 2026-05-21) — S7 cycle revert 동시 작성, 본 ADR 과 sister.
...
- **2026-05-21 Accepted**: operator family v2.0 통일 결정 (사용자 maintainer). ADR-0018 supersede. valkey ADR-0048 + mongodb ADR-0033 와 sister 관계 수립.
