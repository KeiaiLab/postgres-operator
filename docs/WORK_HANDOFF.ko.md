...
| **1** | **동시 쓰기 무중단 실증(native 라우터) + target 영구 승격** | online e2e 는 정적 데이터 검증(라이브 쓰기 포착은 TestCDCLive 별도 증명). shardingMode=native(라우터)에서 클라 쓰기를 라우터 경유로 받아 write-block 이 실제로 막는 무중단 실증 + transient target(rsd-<id>)→영구 ordinal shard 승격(status/failover/메트릭 인식) + 외래키/체크 제약 복제 |
