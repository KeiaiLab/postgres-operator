...
フェーズ詳細(サブタスク、SLO、ADR/RFC 参照): [`docs/roadmap.md`](docs/roadmap.md) および [`ROADMAP.md`](ROADMAP.md)。
...
運用プレイブックは [`docs/operator-guide/deployment.md`](docs/operator-guide/deployment.md) および [`docs/operator-guide/pooler-monitoring.md`](docs/operator-guide/pooler-monitoring.md) を参照してください。

## Production readiness (本番運用準備)

**現状 (0.3.0-alpha.18, 2026-05-12)**: argos Kubernetes クラスターにおいて、ArgoCD Application `platform-data-postgres-operator` は `Synced/Healthy` であり、`PostgresCluster/argos-postgres` は `Ready=True` を報告しています。CloudNativePG とのクロスバリデーションは [`docs/operator-guide/cross-validation-cnpg.md`](docs/operator-guide/cross-validation-cnpg.md) に記載されています。
...
- **P2** — マルチシャードには `ShardRange` CRD + pg-router PoC が必要です ([`docs/sharding/SHARDING.md`](docs/sharding/SHARDING.md))。
...
- Sharding サブシステム (`ShardRange`、`pg-router`、`ShardSplitJob`) は **設計のみ** — 仕様は [`docs/sharding/SHARDING.md`](docs/sharding/SHARDING.md) を参照。ランタイムコードはまだありません。
