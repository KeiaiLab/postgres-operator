...
Helm chart 和 OperatorHub bundle 包含 **8 个自有 CRD**。CRD 状态反映了当前在 production cluster上已 reconcile 的内容:
...
**当前状态(0.3.0-alpha.18,2026-05-12)**:在 production Kubernetes 集群上,ArgoCD Application `platform-data-postgres-operator` 处于 `Synced/Healthy` 状态,`PostgresCluster/postgres` 报告 `Ready=True`。
